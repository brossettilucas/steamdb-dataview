...
# tag_years,tag_prices,tag_scores,tag_reviewcount, dev_years ,dev_prices ,dev_scores ,dev_reviewcount ,datath1 , datath2 , datath4 ,datath5 , datath6 = build_dictionaries()

def get_avg_prices_by_tag(tag_prices):
    ...
    return tag_avg_prices

# NAO USAR !!!!
def get_histogram_avg_price(tag_years,tag_prices):
    tag_avg_prices = get_avg_prices_by_tag(tag_prices)
...
# NAO USAR !!!
def get_bar_avg_price(tag_prices):
    tag_avg_prices = get_avg_prices_by_tag(tag_prices)
    ...
    return BAR_dist_normal_avg_price_genre

# NAO USAR !!!
def get_normal_dist_score(tag_scores):
    ...
    return BAR_dist_normal_score_genre

# NAO USAR !!!
def get_bar_normal_score_genre_div(tag_years,tag_scores):
    tag_frequencies = {}
    for tag, years in tag_years.items():
        total = 0
        for count in years.values():
    ...
    return BAR_normal_score_genre_div
...
def get_line_year(datath1 ,isLog=False):
    ...
    LINE_games_year = px.line(games_per_year, x='Release Year', y='Count', color='Tags', title='Quantidade de Jogos Lançados por Ano por Gênero', log_y= isLog)
